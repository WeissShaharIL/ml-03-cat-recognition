...
export default function App() {
  ...
  const [currentEpoch,    setCurrentEpoch]     = useState(null)  // live epoch metrics
  ...
  const resetAll = async () => {
    ...
    try {
      ...
      setRuns([]); setTrainLog([]); setCurrentEpoch(null)
    ...
    try {
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          if (p.type === 'done')  {
            setTrainLog(prev => [...prev, { text: `✓ Training complete — best val accuracy: ${(p.best_val_acc*100).toFixed(1)}%`, color: t.up }])
            fetchHealth(); fetchRuns()
          }
        }
      }
    ...
    <div style={{ fontFamily: 'system-ui', minHeight: '100vh', background: t.bg, color: t.text, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ borderBottom: `1px solid ${t.border}`, background: t.surface, padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 52, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {health && (
            <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 20, background: health.model_ready ? '#16a34a22' : '#88888822', color: health.model_ready ? t.up : t.muted, border: `1px solid ${health.model_ready ? '#16a34a44' : t.border}` }}>
              {health.model_ready ? '● Model ready' : '○ No model'}
            </span>
          )}
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          {message && <span style={{ fontSize: 12, color: message.type === 'success' ? t.up : t.down }}>{message.text}</span>}
          {resetConfirm ? (
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '220px 1fr 300px', minHeight: 0 }}>
        ...
        <div style={{ borderRight: `1px solid ${t.border}`, padding: '20px 14px', overflowY: 'auto' }}>
          ...
          <SectionHeader title="Model" t={t} />
          {health?.model_ready ? (
            <div style={{ fontSize: 12, color: t.muted, lineHeight: 1.8 }}>
              <div>ResNet18 fine-tuned</div>
              <div>Classes: cat / not_cat</div>
              <div style={{ marginTop: 6 }}>
                <a href="http://localhost:3000" target="_blank" rel="noreferrer" style={{ color: t.accent, fontSize: 11 }}>→ Open Classifier UI</a>
              </div>
            </div>
          ) : (
            <p style={{ color: t.muted, fontSize: 12 }}>No model trained yet.</p>
          )}
        </div>
          ...
          <Card t={t}>
            <div style={{ display: 'flex', gap: 12, alignItems: 'stretch', marginBottom: 12 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <button onClick={runTrain} disabled={training} style={btn(training, '#059669')}>
            ...
            {/* Train log */}
            {trainLog.length > 0 && (
              <div ref={trainLogRef} style={{ maxHeight: 180, overflowY: 'auto', background: t.logBg, borderRadius: 7, border: `1px solid ${t.border}`, padding: '8px 12px', fontFamily: 'monospace', fontSize: 12, lineHeight: 1.7 }}>
        ...
        <div style={{ padding: '20px 16px', overflowY: 'auto' }}>
          <SectionHeader title="Training Runs" t={t} />
            ...
            {runs.map((run, i) => (
              <div key={run.id}>
                <div onClick={() => setExpandedRun(expandedRun === run.id ? null : run.id)}
                  style={{ padding: '10px 12px', border: `1px solid ${t.border}`, borderRadius: expandedRun === run.id ? '8px 8px 0 0' : 8, cursor: 'pointer', fontSize: 12, background: i === 0 ? '#16a34a0d' : t.surface, borderColor: i === 0 ? '#16a34a44' : t.border }}>
                  ...
                  <div style={{ display: 'flex', gap: 10, fontSize: 10, color: t.muted, marginTop: 2 }}>
                    <span>🐱 {run.dataset.cat} cats</span>
                    <span>✗ {run.dataset.not_cat} other</span>
                  </div>
                </div>

                {/* Expanded: per-epoch history */}
                {expandedRun === run.id && (
